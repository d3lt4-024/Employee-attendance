    ...
    function index()
    {
        if (isset($_SESSION['permission'])) {
            if ($_SESSION['permission'] === "manager") {
                ...
                $list_form = $this->Manager->GetLeaDayForm($_SESSION['user_id']);
                $department_list_form = $this->Manager->GetListFormDepartment($user_info[0]["Department_Number"]);
                $department_list_form_amount = $this->Manager->GetTotalAmountFormDepartment($user_info[0]["Department_Number"]);
                $department_list_form_approve_amount = $this->Manager->GetApprovedAmountFormDepartment($user_info[0]["Department_Number"]);
                $department_list_form_pending_amount = $this->Manager->GetPendingAmountFormDepartment($user_info[0]["Department_Number"]);
                $salary = $this->Manager->GetSalary($_SESSION['user_id'], $Month, $Year);
                $all_approve_form_in_month = $this->Manager->GetStartEndApproveForm($_SESSION['user_id'], $Start_Day, $End_Day);
                ...
                $list_form = $this->Employee->GetLeaDayForm($_SESSION['user_id']);
                $salary = $this->Employee->GetSalary($_SESSION['user_id'], $Month, $Year);
                $all_approve_form_in_month = $this->Employee->GetStartEndApproveForm($_SESSION['user_id'], $Start_Day, $End_Day);
                $leave_day_approve_in_month = 0;
                foreach ($all_approve_form_in_month as $key => $value) {
                    $start_date = date_create($value["Start_Date"]);
                    $end_date = date_create($value['End_Date']);
                ...
                $amount_form = $this->User->GetAmountLeaForm();
                $amount_form_app = $this->User->GetAmountLeaFormApp();
                $amount_form_pen = $this->User->GetAmountLeaFormPen();
                $all_form = $this->User->GetAllForm();
